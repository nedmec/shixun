...
optimizer = torch.optim.Adam(model.parameters(), lr=0.0002)  # 更新所有层权重
...
def valid_resnet(model):
    ...
    with torch.no_grad():
        ...
        valid_classification_report = metrics.classification_report(valid_true, valid_pred, digits=4,zero_division=0)
    return valid_pred, valid_true, valid_AUC, valid_acc, valid_avg_loss
def for_test_resnet(best_model_name):
    print('------ Testing Start ------')
    model.load_state_dict(torch.load(best_model_name, weights_only=True), False)
    ...
    test_classification_report = metrics.classification_report(test_true, test_pred, digits=4, zero_division=0)
...
# 测试集
test_acc, test_img, test_true, test_pred = for_test_resnet(best_model_name)
show_batch = 100
